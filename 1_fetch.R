p1_targets <- list(
  ...
  tar_target(p1_watershed_ex,
             st_read(p1_watershed_ext_sf) %>% st_transform(crs=p1_proj) %>% saveRDS('1_fetch/out/watershed_ext_sf.rds')),
  # Read in saline lake
  ...
  tar_target(p1_saline_lakes,
             st_read(p1_saline_lakes_sf) %>% st_transform(crs=p1_proj) %>% saveRDS('1_fetch/out/saline_lakes_sf.rds')),
  ...
  tar_target(p1_gbd_bnd,
             st_read(p1_gbd_bnd_sf) %>% st_transform(crs=p1_proj) %>%  ms_simplify() %>% saveRDS('1_fetch/out/gbd_bnd.rds')),
 #  PAD/land ownership
 ...
 tar_target(p1_states,
            sf::st_read(p1_states_sf) %>% st_transform(crs=p1_proj) %>% ms_simplify() %>% saveRDS('1_fetch/out/states.rds'))
