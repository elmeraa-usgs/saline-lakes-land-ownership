...
gbd_mng_total <- gbd_pad_int |>
  st_drop_geometry() |>
  group_by(Mng_Typ) |>
  summarize(total = as.numeric(sum(area_perc))) |>
  arrange(desc(total))

gbd_cat_total <- gbd_pad_int |>
  st_drop_geometry() |>
  group_by(Mng_Typ) |>
  summarize(total = as.numeric(sum(area_perc))) |>
  arrange(desc(total))

gbd_plot <- gbd_pad_int |>
  left_join(gbd_mng_total)
gbd_plot

gbd_mng_total |>
  ggplot(aes(total, Mng_Typ)) +
